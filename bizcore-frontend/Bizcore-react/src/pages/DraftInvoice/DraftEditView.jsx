...
import DraftDeliveryAddView from './DraftDeliveryAddView';
import DraftConfirmationView from './DraftConfirmationView';
import InvoiceCustomerView from '../Customers/InvoiceCustomerView';
import DraftDeleteConfirmView from './DraftDeleteConfirmView';
import axios from 'axios';
import { toast } from 'react-toastify';

const DraftEditView = ({ setActiveSection, draftInvoiceData, refreshDraftInvoices, setDraftInvoiceData, draftInvoiceId }) => {

    const [selectedCustomer, setSelectedCustomer] = useState(draftInvoiceData?.customer || null);
    ...
    const handleCustomerSelect = (selectedCustomer) => {
        ...
        setDraftInvoiceData({
            ...draftInvoiceData,
            customer: selectedCustomer,  // Update the customer in the draft invoice
        });
    };

    const [itemToDelete, setItemToDelete] = useState(null); // Store item to delete
    const [items, setItems] = useState(draftInvoiceData?.items || []);

    const dropdownRefs = useRef([]); // Ref to keep track of the dropdowns
    ...
    const [dueDate, setDueDate] = useState(draftInvoiceData?.due_date || '');
    ...
    const [gstType, setGstType] = useState(draftInvoiceData.gst_type);
    const [invoiceType, setInvoiceType] = useState(draftInvoiceData?.invoice_type || 'individual');
    const [transportMode, setTransportMode] = useState(draftInvoiceData?.transportation_mode || 'none');
    const [modeOfSupply, setModeOfSupply] = useState(draftInvoiceData?.mode_of_supply || 'direct');
    ...
    const [deliveryDetails, setDeliveryDetails] = useState(draftInvoiceData?.delivery || {});  // Set initial delivery details from draftInvoiceData if available

    const handleDeliveryDetailsChange = (data) => {
        console.log('Received Delivery Details:', data);  // Log to verify data is correct
    ...
    const updateInvoice = async () => {
        ...
        const invalidStockItem = items.find(item => {
            ...
            if (product) {
                // Check if the qty exceeds stock
                const originalItem = draftInvoiceData.items.find(existingItem => existingItem.product === item.product);
        ...
        try {
            ...
            const invoiceData = {
                ...
                delivery: modeOfSupply === 'delivery' ? {
                    ...
                    dispatched_at: deliveryDetails.dispatched_at || '',
                    delivery_notes: deliveryDetails.delivery_notes || '',
                    delivered_at: deliveryDetails.delivered_at || '',
                } : {}
            };

            console.log("Updated Delivery Details: ", deliveryDetails);
            console.log('Request Data:', invoiceData); // Log to verify data before sending it

            const response = await axios.put(
                `http://127.0.0.1:8000/api/draft-invoices/${draftInvoiceData.id}/`,
                invoiceData,
                { headers: { 'Content-Type': 'application/json' } }
            );

            if (response.status === 200) {
                const updatedInvoice = await axios.get(`http://127.0.0.1:8000/api/draft-invoices/${draftInvoiceData.id}/`);
                setDraftInvoiceData(updatedInvoice.data);
            ...
            if (error.response && Array.isArray(error.response.data)) {
                ...
                toast.error(`Failed to update invoice: Remove empty rows`);
            }
        }
    };

    const handleSaveDraft = async () => {
        const isSuccess = await updateInvoice();  // Wait for the result of updateInvoice()

        if (isSuccess) {
            // If the invoice update was successful, show the confirmation view
            showPopupConfirmationView();  // Call the function without any arguments
        } else {
            toast.error('Error saving the invoice. Please try again.');
        }
    };
    ...
    const addItem = () => {
        ...
        refreshDraftInvoices();  // Call the function passed from the parent
    };
    ...
    const handleInputChange = (e, index, field) => {
        ...
        // Handle the 'qty' (quantity) change separately
        if (field === 'qty') {
        ...
        // Handle other fields (e.g., 'rate', 'name', etc.)
        else {
            updatedItems[index][field] = e.target.value;  // Update other fields like 'name', 'rate', etc.
        }
    ...
    const removeItem = async (index) => {
        ...
        try {
            ...
            if (!isEmptyRow(item)) {
                await axios.delete(`http://127.0.0.1:8000/api/draft-invoices/${draftInvoiceData.id}/delete-item/${item.id}/`, {
                    headers: {
    ...
    useEffect(() => {
        setSelectedCustomer(draftInvoiceData?.customer);
        setItems(draftInvoiceData?.items);
    }, [draftInvoiceData]);

    if (!draftInvoiceData || !draftInvoiceData.items) {
        return <div>Loading...</div>;  // Show loading message if data is missing
    }

    useEffect(() => {
        if (draftInvoiceData?.items) {
            setItems(draftInvoiceData.items); // Update state when draftInvoiceData is available
        }
    }, [draftInvoiceData]);
                        ...
                        <div className='flex justify-between items-center bg-gray-200 rounded-lg p-1 w-full'>
                            <div className='flex flex-col '>
                                <div className='flex flex-col p-1 pl-4'>
                                    ...
                                    <p className='text-sm'>{draftInvoiceData?.draft_number}</p>
                                </div>
                                <div className='flex pl-4'>
                                    <h1 className='text-md font-semibold'>Draft Date</h1>
                                    <p className='pl-2'>:</p>
                                    <p className='text-sm pl-1'>{draftInvoiceData?.draft_date}</p>
                                </div>
                                <div className="flex pl-4">
                            ...
                            <div className='flex flex-col text-end p-2 pr-4'>
                                ...
                                {selectedCustomer ? (
                                    <>
                                        <h1 className='text-md'>{selectedCustomer.name || draftInvoiceData?.customer_name}</h1>
                                                ...
                                                <td className="p-1 pt-2 relative w-45">
                                                    ...
                                                    {item.isDropdownVisible && item.filteredProducts.length > 0 && (
                                                        <div
                                                            ...
                                                            className="absolute top-2 w-80 left-45 bg-white border border-gray-300 z-10">
                            ...
                            <div className='flex flex-col bg-white p-4 rounded-lg'>
                                ...
                                <div
                                    ...
                                    <div className='flex flex-col justify-center items-start'>
                                        {selectedCustomer ? (
                                            <>
                                                <h1 className='pl-2 font-semibold text-sm w-50'>{selectedCustomer.name || draftInvoiceData?.customer_name}</h1>
                                                <p className='pl-2 text-sm'>{selectedCustomer.phone || draftInvoiceData?.phone_number}</p>
                                            </>
                                        ) : (
                ...
                isPopupDeliveryAdd && (
                    <div className="fixed inset-0 flex items-center justify-center bg-opacity-50 z-50 ">
                        <div className={`bg-white p-4 rounded-lg shadow-xl w-3/4 ${isFullScreenDelivery ? 'w-full h-full' : ''}`}
                            ...
                            <DraftDeliveryAddView setActiveSection={setActiveSection}
                                deliveryData={draftInvoiceData?.delivery || deliveryDetails || {}}  // Pass the existing delivery data
