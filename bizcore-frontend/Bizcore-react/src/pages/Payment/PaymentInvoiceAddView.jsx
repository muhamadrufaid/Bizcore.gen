...
const PaymentInvoiceAddView = ({ setActiveSection, invoiceId, invoiceDetails, customerId, customerName, closePopupPayment, closePopupConfirmationView }) => {
    ...
    const [balanceDue, setBalanceDue] = useState(invoiceDetails?.remaining_balance || 0.00);
    ...
    useEffect(() => {
        ...
        const calculatedBalanceDue = invoiceDetails?.grand_total - amountPaid;
        setBalanceDue(calculatedBalanceDue);

        // Set payment status based on amount paid and grand total
        if (amountPaid === invoiceDetails?.remaining_balance) {
            setPaymentStatus('paid');
            setDueDate(new Date().toISOString().split('T')[0]);  // Set due date to today for fully paid
        } else if (amountPaid < invoiceDetails?.remaining_balance && amountPaid > 0) {
        ...
        } else if (amountPaid > invoiceDetails?.remaining_balance) {
            setErrorMessage('Amount paid cannot exceed the remaining balance');
        } else {
            setErrorMessage('');
        }
    }, [amountPaid, cashAmount, onlineAmount, paymentMode, invoiceDetails?.remaining_balance]);
    ...
    const handlePaymentSubmit = async () => {
        ...
        const paymentData = {
            ...
            bill_amount: invoiceDetails?.grand_total,
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Optionally, update UI or redirect user after successful payment
                closePopupPayment();
                closePopupConfirmationView();
    ...
    return (
        <div>
            <div className='m-2'>
                <h1 className='font-semibold text-xl text-blue-500'>Payment Details draft</h1>
            </div>
            <div className='mt-2 flex justify-between'>
                <div className='mb-2'>
                    <div className='flex items-center pl-2'>
                        ...
                        <p className='pl-2'>:</p>
                        <p className='text-sm pl-1'>{invoiceId}</p>
                    </div>
                    <div className='flex items-center pl-2'>
                        <h1 className='text-md font-semibold'>Customer</h1>
                        <p className='pl-2'>:</p>
                        <p className='text-sm pl-1'>{customerName || 'Customer name not available'}</p>
                    </div>
                ...
                <div>
                    <div className='flex items-center pr-2'>
                        <h1 className='text-md font-semibold'>You have only <span className='text-red-400 font-bold'>₹{invoiceDetails?.remaining_balance}</span> left to pay</h1>
                    </div>
                </div>
            </div>
...
export default PaymentInvoiceAddView;
